    ...
    public static void init(final Context ctx, final String token, final CameraNativeInitCallback cb) {
        CameraThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                int authStatus = 0;
                try {
                    authStatus = IDcardQualityProcess.init(token);
                } catch (IDcardQualityProcess.AlgorithmOnMainThreadException e) {
                    e.printStackTrace();
                } catch (IDcardQualityProcess.IDLAuthorityException e) {
                    e.printStackTrace();
                }
                if (authStatus != 0) {
