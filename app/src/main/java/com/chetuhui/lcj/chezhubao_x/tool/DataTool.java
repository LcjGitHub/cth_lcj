...
import static com.chetuhui.lcj.chezhubao_x.tool.ConstTool.BYTE;
import static com.chetuhui.lcj.chezhubao_x.tool.ConstTool.GB;
import static com.chetuhui.lcj.chezhubao_x.tool.ConstTool.KB;
import static com.chetuhui.lcj.chezhubao_x.tool.ConstTool.MB;
...
public class DataTool {
     ...
     * @return 130****0000
     */
    public static String hideCarcode2(String carcode) {
        if (carcode.length() != 7&carcode.length() != 8) {
            return "车牌号有误";
        }
        if (carcode.length()==7){
            return carcode.substring(0, 3) + "****" + carcode.substring(6, 7);
        }else if (carcode.length()==8){
            return carcode.substring(0, 3) + "****" + carcode.substring(7, 8);
        }
        return "车牌号有误";
    }
  ...
  public static String getIntValue(String value) {
        ...
        return INT_FORMAT.format(Double.parseDouble(value));
    }
    ...
    public static String changeDistance(double length, boolean displayMeter) {
        if (length < 1000) {
            return Constants.FORMAT_TWO.format(length) + (displayMeter ? "米" : "");
        } else {
            return Constants.FORMAT_TWO.format(length / 1000) + (displayMeter ? "千米" : "");
        }
    }
