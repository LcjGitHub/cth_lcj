...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int TITLE_BG_COLOR = Color.parseColor("#f9f9f9");
    private static int TITLE_TEXT_COLOR = Color.parseColor("#FF000000");
    private static int mTitleTextSize;


    public TitleItemDecoration(Context context, List<CityBean.DataBean> data) {
        super();
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).getPinyin() && !mData.get(position)
                            .getPinyin().equals(mData.get(position - 1).getPinyin())) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(mData.get(position).getPinyin(), 0, mData.get(position).getPinyin().length(), mBounds);
        c.drawText(mData.get(position).getPinyin(),
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (null != mData.get(position).getPinyin() &&
                        !mData.get(position).getPinyin().equals(mData.get(position - 1).getPinyin())) {
