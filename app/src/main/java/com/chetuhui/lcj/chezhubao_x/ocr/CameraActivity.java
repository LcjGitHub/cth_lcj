...
public class CameraActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        confirmResultContainer.findViewById(R.id.confirm_button_re).setOnClickListener(confirmButtonOnClickListener);
    ...
    private void cropAndConfirm() {
        Log.d("CameraActivity", "cropAndConfirm");
        cameraView.getCameraControl().pause();
        updateFlashMode();
        doConfirmResult();
        BaseToast.success("请再选择一次图片");
    ...
    private void doConfirmResult() {
        CameraThreadPool.execute(new Runnable() {
            @Override
            public void run() {
                Log.d("CameraActivity", "doConfirmResult");
                try {
                    FileOutputStream fileOutputStream = new FileOutputStream(outputFile);
                    Bitmap bitmap = ((BitmapDrawable) displayImageView.getDrawable()).getBitmap();
                    bitmap.compress(Bitmap.CompressFormat.JPEG, 100, fileOutputStream);
                    fileOutputStream.close();
                    Log.d("CameraActivity", "run");
                    Log.d("CameraActivity", "contentType:"+contentType);
    ...
    private View.OnClickListener confirmButtonOnClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            Log.d("CameraActivity", " confirmButtonOnClickListener:");
            doConfirmResult();
        }
    };
