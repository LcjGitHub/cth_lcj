...
import com.chetuhui.lcj.myapplication.model.ModelSpider;
import com.chetuhui.lcj.myapplication.tool.ImageTool;
import com.chetuhui.lcj.myapplication.tool.RotateTool;
...
public class CobwebView extends View {
    ...
    private void initEvent() {
        defalutSize = ImageTool.dp2px(defalutSize);
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            double rotateDis = RotateTool.CIRCLE_ANGLE * (Math.abs(max - mFlingPoint) / mPerimeter);
    ...
    private void handleRotate(double rotate) {
        rotate = RotateTool.getNormalizedAngle(rotate);
        mRotateAngle = rotate;
        invalidate();
    }
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        //蛛网等级颜色
        mSpiderLevelColor = ImageTool.changeColorAlpha(mSpiderLevelStrokeColor, (255 / 2));
        //是否需要 蛛网等级描边
        ...
        //标题字体大小
        mSpiderNameSize = a.getDimensionPixelSize(R.styleable.RxCobwebView_spiderNameSize, ImageTool.dp2px(16));
        a.recycle();
    }
    ...
    private void initLevelPoints() {
        ...
        for (int i = mSpiderMaxLevel; i > 0; i--) {
            ...
            paint.setColor(ImageTool.changeColorAlpha(mSpiderColor, (255 / (mSpiderMaxLevel + 1) * (mSpiderMaxLevel - i - 1) + 255 / scale) % 255));
            //设置实心
    ...
    private void drawCobweb(Canvas canvas, int index) {
        ...
        if (mSpiderLevelStroke) {
            if (scoreStrokePaint == null) {
                scoreStrokePaint = new Paint();
                scoreStrokePaint.setColor(ImageTool.changeColorAlpha(levelPaintList.get(mSpiderMaxLevel - 1).getColor(), 50));
                scoreStrokePaint.setStyle(Paint.Style.STROKE);
                scoreStrokePaint.setAntiAlias(true);
                if (mSpiderLevelStrokeWidth > 0) {
                    scoreStrokePaint.setStrokeWidth(mSpiderLevelStrokeWidth);
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            double rotate = mRotateAngle;
            double dis = RotateTool.getRotateAngle(new PointF(e2.getX() - distanceX, e2.getY() - distanceY)
