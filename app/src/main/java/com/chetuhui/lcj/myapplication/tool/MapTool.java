...
import com.chetuhui.lcj.myapplication.model.Gps;
...
public class MapTool {
    ...
    public static void openMap(Context mContext, Gps gpsFrom, Gps gpsTo, String storeName) {
        //检测设备是否安装高德地图APP
        if (PackageManagerTool.haveExistPackageName(mContext, Constants.GAODE_PACKAGE_NAME)) {
            MapTool.openGaodeMapToGuide(mContext, gpsFrom, gpsTo, storeName);
            //检测设备是否安装百度地图APP
        } else if (PackageManagerTool.haveExistPackageName(mContext, Constants.BAIDU_PACKAGE_NAME)) {
            MapTool.openBaiduMapToGuide(mContext, gpsTo, storeName);
            //检测都未安装时，跳转网页版高德地图
        } else {
            MapTool.openBrowserToGuide(mContext, gpsTo, storeName);
        }
    }
    ...
    public static void openGaodeMapToGuide(Context mContext, Gps gpsFrom, Gps gpsTo, String storeName) {
        ...
        Gps gps = LocationTool.GPS84ToGCJ02(gpsFrom.getLongitude(), gpsFrom.getLatitude());
        Gps gps1 = LocationTool.GPS84ToGCJ02(gpsTo.getLongitude(), gpsTo.getLatitude());
    ...
    public static void openBaiduMapToGuide(Context mContext, Gps gps, String storeName) {
        Intent intent = new Intent();
        Gps gps1 = LocationTool.GPS84ToGCJ02(gps.getLongitude(), gps.getLatitude());
        Gps gps2 = LocationTool.GCJ02ToBD09(gps1.getLongitude(), gps1.getLatitude());
